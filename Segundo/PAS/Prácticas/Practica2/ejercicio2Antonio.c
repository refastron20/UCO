...
switch(rf=fork())
{
	...
	case 0 : // código que ejecuta el hijo
		...
		if (write(tubo2[1], resultado, sizeof(resultado)) == -1)
		{
	...
	default:// código del proceso padre
		...
		{
			perror("Error en close"); //entra aqui no se porque
			exit(EXIT_FAILURE);
		}
		...
		if (close(tubo2[1]) == -1) 
		{
	 	...
	 	strcpy(primero,strcat(primero,tercero));

	 	// Escribimos datos en la tubería
		if (write(tubo1[1], primero, sizeof(tercero)) == -1)
		{
			perror("Error en write"); 
			exit(EXIT_FAILURE);
		}
