...
int main(){
        ...
        if(i==0){
            ...
            for(int j=0;j<(NPROC-1);j++){
                ...
                if(procesos2[j].t_ejec<min){
                    ...
                    aux=j;//permite que si encuentra dos mínimos no elimine el primero
                }
            }
            procesos2[aux].t_ejec=MAX;//no se permite que vuelva a salir

            for(int j=1;j<NPROC;j++){

                if(procesos[j].t_ejec==min){
                    valor=j; //para saber que proceso es
                }
            }
    ...
    //imprimo los resutados no ordenados sino de A,B,C,D,E
