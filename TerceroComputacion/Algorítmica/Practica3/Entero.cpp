...
Entero Entero::multiplicar(const Entero &n){
	Entero w,x,y,z;
	Entero r,p,q;
	std::string u=this->getEntero();
	std::string v=n.getEntero();
	Entero *aux= new Entero();
	if (u.size()<=this->getTamMax() and v.size()<=this->getTamMax()) {
		aux->setEntero(atoi(u.c_str())*atoi(v.c_str()));
		return *aux;
	}
	if (u.size()>v.size()) {
	...
	Entero mult1=w.multiplicar(y);
	string saux=mult1.getEntero();
	agregarCerosFinal(saux,2*s);
	mult1.setEntero(saux);
	Entero mult2=w.multiplicar(z)+x.multiplicar(y);
	saux=mult2.getEntero();
	agregarCerosFinal(saux,s);
	mult2.setEntero(saux);
	Entero mult3=x.multiplicar(z);
	*aux=mult1+mult2+mult3;
	return *aux;
}
