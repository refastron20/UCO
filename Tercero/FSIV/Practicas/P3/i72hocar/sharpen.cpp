...
void convolve(const cv::Mat& in, const cv::Mat& filter, cv::Mat& out, int border_type){
  ...
  float suma;
  for (int i = (PadRows/2); i < in.rows+(PadRows/2); i++) {
    for (int j = (PadCols/2); j < in.cols+(PadCols/2); j++) {
      suma=0;
      for (int a = 0; a < filter.rows; a++) {
        for (int b = 0; b < filter.cols; b++) {
          suma+=filter.at<float>(a,b)*in.at<float>(i-(PadRows/2)+a,j-(PadCols/2)+b);
        }
      }
      aux.at<float>(i,j)=suma;
    }
  }
  ...
  for (int i = 0; i < out.rows; i++) {
    for (int j = 0; j < out.cols; j++) {
      out.at<float>(i,j)=aux.at<float>(i+(PadRows/2),j+(PadCols/2));
    }
  }
}
