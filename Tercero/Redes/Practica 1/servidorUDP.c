...
int main ()
{
   ...
   ----------------------------------------------------------------------*/
    ...
    /* -----------------------------------------------------------------
        ...
        if (strcmp("DAY",Datos) == 0) {
          strftime(Datos, 320, "%A, %d de %B de %Y", stTm);
					printf("Datos:%s\n",Datos );
        } else if(strcmp("TIME",Datos) == 0){
					strftime(Datos, 320, "%H:%M:%S", stTm);
        } else if(strcmp("DAYTIME",Datos)==0){
					strftime(Datos, 320, "%A, %d de %B de %Y; %H:%M:%S", stTm);
				}else{
					strcpy(Datos, "Error: mensaje no reconocido");
				}
      	/* ------------------------------------------------------------------
				Devolvemos la cadena modificada al cliente
				--------------------------------------------------------------------*/
			sendto (Socket_Servidor, Datos, sizeof(Datos), 0,
			(struct sockaddr *) &Cliente, Longitud_Cliente);
